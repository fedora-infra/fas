...
class ViewsAdminFunctionalTests(BaseTest):
    ...
    def test_get_admin(self):
        ...
        # TODO: need a way to get the values of the subjects above and assert

    def test_admin_dump_people(self):
        self.login_helper()
        res = self.testapp.get('/settings/dump/people', status=200)
        json_res = json.loads(res.body)
        self.assertEqual(json_res['total'], 3)  # total active usesrs
        self.assertEqual(len(json_res['rows']), 5)  # total all users
    ...
    def test_admin_dump_groups(self):
        ...
        self.assertEqual(len(json_res['rows']), 7)
