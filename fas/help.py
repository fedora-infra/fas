...
class Help(controllers.Controller):
    help = { 'none' :               [_('Error'), _('<p>We could not find that help item</p>')],
            ...
            'user_ssh_key' :        [_('Public RSA, ECDSA SSH Key'), _('<p>Many resources require public key authentication to work.  By uploading your public key to us, you can then log in to our servers.  Type "man ssh-keygen" for more information on creating your key (it must be an RSA or ECDSA key).  Once created you will want to upload ~/.ssh/id_rsa.pub or ~/.ssh/id_ecdsa.pub. <br /><br />If you wish to login through several hosts, each with their own public key, you can create a concatenated file of public ssh keys and upload it in lieu of the individual ssh public key. <br /><br />"Warning: In case of having ECDSA key please upload the two types of keys because some of our servers may not accept ECDSA key."</p>')],
