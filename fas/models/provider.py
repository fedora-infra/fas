...
def get_group_by_id(session, id):
    ...
    query = session.query(Groups).filter(id == id)
    return query.first()


def get_group_by_name(session, name):
    """ Retrieve a specific Groups by its name. """
    query = session.query(Groups).filter(name == name)
    return query.first()
...
def get_grouptype_by_id(session, id):
    ...
    query = session.query(GroupType).filter(id == id)
    return query.first()
